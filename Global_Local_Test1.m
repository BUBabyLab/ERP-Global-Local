...
datafilename = strcat('Global_Local_Test1_',num2str(subNo),'.dat'); % name of data file to write to
imagefilename = 'ImageList.txt';                           % study list
cuefilename  = 'CueList.txt';                            % test list

% check for existing result file to prevent accidentally overwriting
% files from a previous subject/session (except for subject numbers > 998):
%if subNo<998 && fopen(datafilename, 'rt')~=-1
%    fclose('all');
...
%%%%%%%%%%%%%%%%%%%%%
    ...
    fileID = fopen('Test_Script.txt','r');
    
    ImageList = textscan(fileID,'%s','delimiter','\n');
    fclose(fileID);
    
    %  Parse items out of each line of text from file
    for j in range 1:
        InitRespList = strsplit(ImageList{j,1},' ');
    end
...
%try
    ...
    for trial = 1:numtrials
        ...
        for phase=1:2 % 1 is cue phase, 2 is test phase                

            % Setup experiment variables etc. depending on phase:
            if phase==1 % cue phase
                ...
                message = 'Cue phase ...\nLook at the image ... press _n_ when it disappears ...\n... press mouse button to begin ...';
